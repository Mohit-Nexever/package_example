...
class CustomRichTextWidget extends StatelessWidget {
  ...
  const CustomRichTextWidget({super.key, required this.actualText, required this.query, this.textStyle, this.highlightTextStyle});
  ...
  Widget build(BuildContext context) {
    ...
    final result = splitTextAndFindQueryPosition(actualText, query.trimRight());
    ...
    for (int i = 0; i < splitText.length; i++) {
      textSpans.add(
        TextSpan(text: splitText[i], style: i == queryIndex ? (highlightTextStyle ?? defaultHighlightTextStyle) : (textStyle ?? defaultTextStyle)),
      );
    }
...
Map<String, dynamic> splitTextAndFindQueryPosition(String actualText, String query) {
  ...
  if (query.trim().isEmpty || !actualText.toLowerCase().contains(query.toLowerCase())) {
  ...
  if (queryIndex != -1) {
    ...
    String queryMain = getCaseSensitiveSubstring(actualText, query);
...
String getCaseSensitiveSubstring(String text, String word) {
